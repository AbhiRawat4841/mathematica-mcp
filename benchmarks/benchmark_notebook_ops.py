...
def run_benchmarks():
    ...
    r = benchmark_operation(conn, "execute_code_simple", "execute_code", {"code": "1 + 1"}, iterations=10)
    if r:
        results.append(r)
...
def save_results(results, filename):
    ...
    if results:
        output = {"timestamp": time.strftime("%Y-%m-%d %H:%M:%S"), "results": results}
...
if __name__ == "__main__":
    ...
    if results:
        ...
        filename = os.path.join(os.path.dirname(__file__), f"benchmark_results_{suffix}.json")
