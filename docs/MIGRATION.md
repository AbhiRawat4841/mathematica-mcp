...
- **Addon protocol is now `4`** (1.1.1): reinstall the addon and restart Mathematica (see above). A stale addon is detected and reported by `status()`.
- **Front-end evaluations return an honest pending status.** `execute_code(mode="frontend")`, `evaluate_cell`, and `evaluate_selection` no longer claim success with empty output while the notebook is still computing; they return `status: "evaluation_pending"` with `evaluation_complete: false`, and the output cell lands in the notebook right after the call returns (re-check with `get_cells` / `cells`). Anything scripted against the old `evaluated: true` shape must read the new fields.
- **Interactive code auto-routes to the front end** (1.1.2): `Manipulate`/`Dynamic`/`Animate` sent to a notebook without an explicit mode renders as a live panel via front-end evaluation. Explicit `mode`/`style` still wins.
- **Lean `evaluate` responses default to `compact`**: set `MATHEMATICA_RESPONSE_DETAIL=standard` to restore the full pre-1.1 response shape. Failures always keep the complete diagnostic shape.
...
|-------------|-----------------|
| `get_mathematica_status`, `get_feature_status` | `status()` (`get_session_brief`'s recent-error/routing-advice payload is a routing-memory feature, not part of `status()`) |
...
- **`state_delta` is scoped.** Addon responses carry a `state_delta` (`notebook` / `cell_count` / `kernel_busy`) only on notebook-touching commands (plus `batch_commands`), not on every response; `kernel_busy` reports the front end's `Evaluating` state (since 1.1.1: read from the command's target notebook).
