...
class TestCalculus:
    ...
    def test_gaussian_integral(self):
        ...
        assert result["success"] is True
        assert "Sqrt[Pi]" in result["output"]

    def test_derivative(self):
        """Test differentiation."""
        result = execute_in_kernel("D[Sin[x], x]")
        assert result["success"] is True
        assert "Cos[x]" in result["output"]
    ...
    def test_taylor_series(self):
        ...
        assert result["success"] is True
        assert "x^2/2" in result["output"]
        assert "x^3/6" in result["output"]
...
class TestSpecialFunctions:
    ...
    def test_zeta_function(self):
        ...
        assert result["success"] is True
        assert "Pi^2/6" in result["output"]
...
class TestNumericalComputation:
    ...
    def test_numerical_integration(self):
        ...
        assert "0.31" in result["output"] or float(result["output"]) > 0.3

    def test_find_root(self):
        """Test root finding."""
        result = execute_in_kernel("FindRoot[Cos[x] == x, {x, 0}]")
        assert result["success"] is True
        assert "0.739" in result["output"]
...
class TestEdgeCases:
    ...
    def test_division_by_zero(self):
        ...
        assert "$Failed" in result["output"] or "ComplexInfinity" in result["output"]
...
class TestJSONParsingFix:
    ...
    def test_series_expansion_original_bug(self):
        ...
        assert result["success"] is True
        assert "SeriesData" in result["output"]
    ...
    def test_series_with_normal(self):
        ...
        assert result["success"] is True
        assert "x^5/120" in result["output"]
    ...
    def test_multiple_line_output(self):
        ...
        assert "{5, 25, 125}" in result["output"]
